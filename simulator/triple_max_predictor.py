...
class TripleMaxPredictor(StatefulPredictor):
    ...
    def Predict(self, vm_states_and_num_samples):
        ...
        for vm_state_and_num_sample in vm_states_and_num_samples:
            if len(vm_state_and_num_sample.vm_state.limit) > 0:
                vms_limits.append(0.9*vm_state_and_num_sample.vm_state.limit[0])

        borg_predicted_peak = 0.9*sum(vms_limits)

        return max(borg_predicted_peak, N_sigma_predicted_peak * current_total_limit, RC_like_predicted_peak * current_total_limit)
