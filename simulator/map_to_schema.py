...
def MapVMSampleToSchema(vm_sample):
    if vm_sample is not None:
        updated_vm_sample = {
            ...
            "avg_cpu_usage": json.loads(vm_sample.get("average_usage", {})).get("cpus", -1),
            "avg_memory_usage": json.loads(vm_sample.get("average_usage", {})).get("memory", -1),
            "max_cpu_usage": json.loads(vm_sample.get("maximum_usage", {})).get("cpus", -1),
            "max_memory_usage": json.loads(vm_sample.get("maximum_usage", {})).get("memory", -1),
            "random_sample_cpu_usage": json.loads(vm_sample.get("random_sample_usage", {})).get(
                "cpus", -1
            ),
            "assigned_memory": vm_sample.get("assigned_memory", -1),
            "sample_rate": vm_sample.get("sample_rate", -1),
            "p0_cpu_usage": dict(
                enumerate(ast.literal_eval(vm_sample.get("cpu_usage_distribution", "[]")))
            ).get(0, -1),
            "p10_cpu_usage": dict(
                enumerate(ast.literal_eval(vm_sample.get("cpu_usage_distribution", "[]")))
            ).get(1, -1),
            ...
            "p91_cpu_usage": dict(
                enumerate(ast.literal_eval(vm_sample.get("tail_cpu_usage_distribution", "[]")))
            ).get(0, -1),
            "p92_cpu_usage": dict(
                enumerate(ast.literal_eval(vm_sample.get("tail_cpu_usage_distribution", "[]")))
            ).get(1, -1),
